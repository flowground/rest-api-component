...
function processAction(msg, cfg) {
    ...
    if (headers && headers.length && formattedMethod !== methodsMap.GET) {
        requestOptions.headers = headers.reduce(
            (headers, header) => {
                return {
                    ...headers,
                    [header.key]: jsonata(header.value).evaluate(msg.body)
                };
            }, { ...(requestOptions.headers || {}) })
    }
    ...
    console.log('JSON.stringify(requestOptions)', JSON.stringify(requestOptions));
