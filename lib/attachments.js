...
function addAttachment(msg, name, body, contentLength, contentType) {
  return getUrlsManualy().then(result => {
    ...
    if(contentType.includes('application/xml')) {
        debug('uploadXml is about to execute');
        try {
          return uploadXml(result);
        } catch (error) {
          console.log(`error: trying to rebound`);
          this.emit('rebound', error);
        }
    }
    else {
  ...
  async function getUrlsManualy() {
    ...
    const signedUrl = attachmentClient.makeRequest({
      method: 'POST',
      url: '/v2/resources/storage/signed-url',
    });

    return { put_url: signedUrl };
  }
  ...
  async function uploadXml(urls) {
      ...
      debug('Trying to insert xml: %j', requestBody);

      await attachmentClient.makeRequest(requestBody);
